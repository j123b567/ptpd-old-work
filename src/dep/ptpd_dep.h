...
#define PERROR(x, ...) \
	do { \
		...
		FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, errcode, 0, errbuf, 200, NULL); \
		message(LOG_ERR, x ": %d - %s\n", ##__VA_ARGS__, errcode, errbuf); \
	} while(0)
#else
...
#if defined(SOCKETS_WSA2_RTSS)
Boolean netInitWSA2Rtss(NetPath * netPath);
Boolean netShutdownWSA2Rtss(NetPath * netPath);
int netSelectWSA2Rtss(TimeInternal * timeout, NetPath * netPath);
ssize_t netRecvEventWSA2Rtss(Octet *buf, TimeInternal *time, NetPath * netPath);
ssize_t netRecvGeneralWSA2Rtss(Octet * buf, NetPath * netPath);
#endif
...
int inet_aton(const char* cp, struct in_addr* inp);
Boolean netFindInterfaceRtss(NetPath * netPath, Octet * ifaceName, UInteger8 * communicationTechnology,
    Octet * uuid, struct in_addr * ifaceAddr);
void tieRtssClock (void);
void getTimeRtss(TimeInternal * time);
void setTimeRtss(TimeInternal * time);
Boolean adjFreqRtss(Integer32 adj);
#endif
...
#if defined(WIN32) && !defined(UNDER_RTSS)
int setitimerWin32(const struct timeval * tv, void (*callback)(int));
void catch_alarm(int sig);
#endif

#if defined(UNDER_RTSS)
int setitimerRtss(const TimeInternal * tv, void (*callback)(int));
void catch_alarm(int sig);
#endif
/** \}*/
